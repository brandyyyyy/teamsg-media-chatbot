...
const TOOL_SPECS = [
  ...
  },
  {
    name: 'get_schedule',
    description:
      'Get competition schedule/results rows, optionally filtered by sport, status (Scheduled/Completed), athlete name, or a specific calendar date. Rows include opponentAthleteName/opponentCountry for head-to-head events (e.g. Bowls, combat sports) - blank for individual-performance events with no direct opponent (e.g. Swimming). For "next N hours"/"today"/"tomorrow" RELATIVE-to-now questions, use get_upcoming_schedule instead (it knows the real current time, which you do not); use this tool\'s "date" filter only for a specific, already-named calendar date the user gave you (e.g. "what\'s on 23 July").',
    parameters: {
      type: 'object',
      properties: {
        sport: { type: 'string', description: 'Broad sport category as tracked live (e.g. "Aquatics", "Athletics"), not a specific discipline.' },
    ...
    parameters: {
      ...
      properties: {
        ...
        groupBy: {
          type: 'string',
          description:
            'Field to group and count by. historical: athleteName, sport, sportGroup, games, year, medal, event, country. schedule: athleteName, sport, event, round, medal, recordType, status, date, opponentAthleteName, opponentCountry (head-to-head events only - blank for individual events). contingent: athleteName, sport, event, eligibility, status, debutantStatus (literal "Yes"/"No").',
        },
        filters: {
          type: 'object',
